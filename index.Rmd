...
```{r tabla-plot, include=FALSE}
plot_dinamico <-  pob_tot_censos |> 
  select(Pob, Censo) |> 
  group_by(Censo) |> 
...
```{r grafico-dinamico1, include=FALSE}
grafico1b <-  plotly::plot_ly(plot_dinamico, x= ~Censo, y= ~Poblacion_Total, name = "Nombre", type = "bar")
```


```{r grafico-dinamico2}
# grafico1b <- grafico1b |> 
#   layout(title = "Poblacion Total Santiago del Estero",
#          xaxis = list("Rondas Censales"),
#          yaxis = list("Población Total"))
...
> Ej_ **86** Santiago del Estero **007** Aguirre y **030** Malbrán => 85007030

Así, un codigo de 5 posiciones indica un Departamento (y uno de 7 podra indicar ´la fraccion, y un de 9 un radio censal), y uno de 8 posiciones a la localidad, puesto o paraje. Esta es la maxima desagregacion posible a una entidad unica e inequivoca.Conforme al sistema de codigos geograficos o Unidades Geoestadisticas, para mas consulte [INDEC](https://geoservicios.indec.gob.ar/codgeo/index.php?pagina=inicio) 
...
-	Radios (Urbanos, Envolventes urbanas)
-	Poblacion y estimaciones por Fracciones
-	por Departamentos
-	Pre-censo viviendas
...
-	Desmontes Redd+ 
-	MAPBioMas
-	Ordenamiento Territorial 2009 y 2015
- Stock pecuario
- Estimaciones agricolas

Capas propias DPE Operativos:

-	Cartografía censal (pais86) de 2001, 2010 y de 2022
...
### Ley de Estadistica SEDES

El trabajo y los proyectos de la DGEyC se encuentran amparados por la Ley 6745 de 6 julio de 2005. [LEY DE ESTADISTICAS Y CENSOS](https://drive.google.com/file/d/17GARtpDxzqrmU3eJ0Eti71QNvpLs6iEN/view?usp=sharing)
...
La **DGEyC es el organismo rector del Sistema Estadistico, Demografico, Economico y Social de la Provincia (SEDES)** responsabilidad y facultades potencian el rol de la DGEyC

> **ART. 3º.** - Créase el Sistema Estadístico Demográfico, Económico y Social de la Provincia **(SEDES)**, el que estará integrado por la información producida por los siguientes organismos o dependencias:
    
  a) La Dirección General de Estadísticas y Censos, será el  organismo rector del sistema, que ejercerá la coordinación del SEDES
  b) Los servicios estadísticos de los Organismos Centralizados y Descentralizados de la Administración Pública Provincial
  c) Los servicios estadísticos de Municipios que adhieran a la presente Ley.
  d) Los servicios estadísticos de organismos nacionales con asiento en la Provincia, en tanto medie común acuerdo con los mismos.
  e) Los servicios estadísticos de los Entes Reguladores con asiento en la Provincia, en tanto medie común acuerdo con los mismos.
  f) Los servicios Estadísticos de todo otro organismo que a juicio de la Dirección General de Estadísticas y Censos, o del Organismo que establezca el Poder Ejecutivo, deba incorporarse al Sistema.
...
SIGSE de la [DGEyC](http://www.estadisticasde.gob.ar/institucional.html) y la [Coop. Cambalache](https://cambalache.coop.ar/) mantienen en desarrollo un proyecto de Visor Webmap. La DGEyC busca a trabes de esta interfaz que los socios e interesados en el proiyecto SIGSE puedan acceder a capas de informacion geografica publicas o con acceso restringido, interoperar, y desarrollar nuevos productos.


**-- Acceda al visor en desarrollo** [GeoNube - DGEyC](https://geonube.com.ar/visor/ide_sde) **--**  



![geonube](geonube.png){#id .class width=70% height=70%} 


### Ordenamiento Territorial

```{r import-data-sigse, include=FALSE}
#pozos
...
Modelamiento estadistico mediante uso de Variables sintomaticas (padrones electorales, matricula escolar, registros de usuarios de servicios), y de informacion basada en Geoprocesos y Teledetección.
